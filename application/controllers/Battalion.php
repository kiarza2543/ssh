...
class Battalion extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'ข้อมูลพื้นฐาน';
    ...
    */
	...
	{  
		...
		$this->form_validation->set_rules('battalion_name', 'Battalion name', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$update = $this->model_battalion->update($data, 1);
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Successfully created');
        		redirect('battalion/', 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('battalion/index', 'refresh');
        	}
        }
        else {
        	...
        	$this->data['battalion_data'] = $this->model_battalion->getBattalionData(1);
			$this->render_template('battalion/index', $this->data);			
        }	
